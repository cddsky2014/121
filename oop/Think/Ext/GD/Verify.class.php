 ...
 */
	private function session($name='',$value='') {
			$prefix   =  $this->C('SESSION_PREFIX');
			if(is_array($name)) { // session初始化 在session_start 之前调用
				if(isset($name['prefix'])) $this->C('SESSION_PREFIX',$name['prefix']);
				if($this->C('VAR_SESSION_ID') && isset($_REQUEST[$this->C('VAR_SESSION_ID')])){
					session_id($_REQUEST[$this->C('VAR_SESSION_ID')]);
				...
				if(isset($name['type']))            $this->C('SESSION_TYPE',$name['type']);
				if($this->C('SESSION_TYPE')) { // 读取session驱动
					$type   =   $this->C('SESSION_TYPE');
				...
				// 启动session
				if($this->C('SESSION_AUTO_START'))  session_start();
			}elseif('' === $value){ 
				if(''===$name){
					// 获取全部的session
    ...
    public function check($code, $id = '') {
        ...
        // session 过期
        if($this->time - $secode['verify_time'] > $this->expire) {
    ...
    public function entry($id = '') {
        ...
        $secode['verify_time'] = $this->time;  // 验证码创建时间
