...
/*无限分类*/
function infinite_class($cates,$fid=array("0","fid","cid"),$level=0){
	...
	foreach($cates as $vo){
		if($fid[0]== $vo[$fid[1]]){
			$vo["level"]=$level;
			$classes[]=$vo;
			infinite_class($cates,array($vo[$fid[2]],$fid[1],$fid[2]),$level+1);
		}		
	}
	return $classes;
}
...
/*无限分类下拉菜单*/
function infinite_select($data,$name,$value,$text,$class='',$fid=false){
	$html="<select class='{$class}' name={$name}><option value=0>顶级菜单</option>";	
	foreach($data as $k=>$v){
		if($fid==$v[$value]){
