...
*/
function upload($ctrl_name,$path = "./upload/",$allow_type = array("jpg","png","gif"),$limit_size=2){
	$file_name = $_FILES[$ctrl_name]["name"];//文件名
	//2、判断是否有文件上传
	//if($file_name==""){
	...
	if(in_array($pfix,$allow_type)==false){
		//exit("非法文件类型，允许上传的类型".implode(",",$allow_type)."!<a href='./upload.php'>返回</a>");
		return 2;
	}
	//5、判断错误类型
	$error = $_FILES[$ctrl_name]["error"];
	if($error!=0){
	...
	//6、判断临时文件是否存在
	$tmp_name = $_FILES[$ctrl_name]["tmp_name"];//获取临时文件 
	if(is_file($tmp_name)==false){
		//exit("文件上传失败(临时文件不存在)!<a href='./upload.php'>返回</a>");
		return 4;
	}
	//判断文件上传大小是否超出限制
	$size = $_FILES[$ctrl_name]["size"];
	$max_size = $limit_size*1024*1024;//2M php.ini 200M
	if($size>$max_size){
...
var_dump(upload("file1","./up/",array("png","php"),2));
?>